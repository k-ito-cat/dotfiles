...
description: project-documents/<project>/prototypes 配下に、使い捨て前提の高品質なブラウザプロトタイプを作成・修正するときに使う。仕様・デザインで考慮できていない UI/UX 体験の穴、画面状態、UI スタック、インタラクションを具体確認するため、軽量化より品質と網羅性を優先する。
---
...
- 方針: `project-documents/<project>/prototypes/README.md`
- ひな形: `project-documents/<project>/prototypes/_template/`
- 正本テンプレート: `project-documents/_template/prototypes/`

## 手順

1. 対象プロジェクトと確認したい体験、画面状態、操作導線を特定する。
2. 関連する `specs/` と `design/` を確認し、プロトタイプで扱う範囲の正本を特定する。
3. `project-documents/<project>/prototypes/_template` がなければ、`project-documents/_template/prototypes/_template` から用意する。
4. `_template` を `prototypes/<prototype-name>` にコピーする。
5. README またはプロトタイプ内に目的、確認観点、未検証のリスクを短く残す。
...
10. プロトタイプ作成中・作成後に判明した仕様やデザインの不足を整理し、必要な正本文書を更新する。
...
- プロトタイプは `project-documents/<project>/prototypes/` 配下に置く。
