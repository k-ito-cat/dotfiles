...
description: Use when the user says `pdocs`, `project-docs`, or wants to set up or evolve project documentation under project-documents. Set up documentation from project-documents/_template when needed, connect the app repository's `docs` path with a symlink, then guide user interviews to fill only necessary specifications in a development-safe order.
---
...
4. `project-documents/README.md` と `_template/` を確認する。
...
1. `specs/product.md`
   - 何を作るか、誰向けか、ゴール / 非ゴール、MVP 範囲
2. `specs/glossary.md`
   - 用語、概念、表示名、データ名
3. `specs/tech.md`
   - 技術選定、制約、運用前提
4. `specs/structure.md`
   - repository 構造、責務分割、依存方向
5. `specs/api.md`, `validation.md`, `error-policy.md`
   - データの流れ、入力制約、失敗時の扱い
6. `design/DESIGN.md`, `screens.md`
   - 仕様として必要な体験、画面候補、デザイン前提
7. `test.md`, `qa.md`, `release.md`, `legal.md`
   - 必要になった段階だけ
...
- 仕様書を網羅的に埋めることを目的にしない。
  ...
  - 対象例: legal、release、qa、詳細な error policy、将来拡張の API など、現時点で判断材料や実装予定がない項目。
