...
- 実装の対象の仕様（機能要求 `FRQ`、品質条件 `NFR`、画面 `SCR` など）のステータスが `DS03`（決定）か。`DS01`（未定義）や `DS02`（仮決定）の仕様に対して実装していないか。仮決定のまま進める場合は、その理由と見直す時期が記されているか。
- テストが、どの機能要求の受け入れ条件と、どの品質条件の最低ラインを検証しているかを、ID で追えるか。検証しているテストが無い受け入れ条件・品質条件が残っていないか。
- 変更の性質に対して、必要なテストの手法が抜けていないか。ロジックは unit、境界をまたぐ挙動は integration、主要な導線は E2E、画面の状態と部品の表示はコンポーネントテストか stories、見た目の変更は visual regression、操作の到達性は a11y の確認を基準とし、必要なのに行っていないものを指摘する。プロダクトREADMEの成果物の一覧で「UI 部品の表示状態」が作成済みなら、stories での確認も対象にする。
