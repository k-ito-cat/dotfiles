...
- メール変更、退会、認証手段削除など重要操作で、直前ログインまたは追加認証を要求しているか。
- メールアドレス列挙防止のため、登録時、ログイン失敗時、パスワード再発行時のメッセージを統一しているか。
- ユーザーが管理する cloud account (hosting、DB、object storage、registrar、mail provider) で二要素認証が有効か。
...
- 「メールアドレスは登録されていません」等の文言で登録済みアドレスが特定できる。
- 個人開発で運用 cloud account に 2FA を設定していない。
...
- Cookie name prefix (`__Host-`, `__Secure-`) の採用を検討しているか。
- Cookie の `Domain` 属性を過度に広げてサブドメイン脆弱性を生んでいないか。
...
- 正規表現にマルチラインフラグや改行などのバイパス余地がないか。
- username / handle / slug 入力で、フレームワーク予約パス (`/_ah`, `/api` 等) との衝突、リザーブド語 (`admin`, `contact`, `login` 等)、数字のみ、アンダースコア開始など事故になりやすい入力を弾いているか。
- レスポンスヘッダ (Location、Set-Cookie、Content-Disposition 等) にユーザー入力を直接埋め込んでいないか。
- error response や error page に stack trace、SQL エラー、内部パス、framework version などの内部情報を含めていないか。
- object storage のディレクトリリスト (S3 / R2 / GCS のバケット index) が公開されていないか。
...
- 公開 object storage のバケットルートが index 可能で、他ユーザーのファイル名が列挙できる。
- 500 ページが framework デフォルトの stack trace 付き応答のまま公開される。
...
- CORS/CSP/security headerの設定への参照と、設定だけでは伝わらない例外。採用値は転記しない
