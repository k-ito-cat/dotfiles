...
1. `specs/`
   - 先に `docs/README.md` の docs 全体の正本分担と更新条件を確認し、そのうえで `_template/specs/README.md` の文書地図に従って今必要なプロダクト判断、要件、技術判断、品質判断だけ扱う
2. `design/`
   - 先に `docs/README.md` の design の責務を確認し、そのうえで `_template/design/README.md` の文書地図に従って必要な体験、画面候補、デザイン前提だけ扱う
3. `prototype/`
   - 先に `docs/README.md` の prototype から戻す基準を確認し、そのうえで `_template/prototype/README.md` の方針に従って操作感の検証が必要になった段階だけ扱う
...
入力条件が不足している場合は、design docs を更新せず、仕様論点としてユーザーと整理する。入力条件が揃い、UI / UX 判断が必要な場合だけ `design-workflow` に渡す。
