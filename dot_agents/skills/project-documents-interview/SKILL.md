...
この skill の目的は、template の空欄を埋めることではない。専門家観点で論点を網羅的に発見し、各論点をステータス表のいずれか、または振り分け先（`別文書へ送る`、`prototypeで検証`）に分類し、品質、意図、判断の再現に必要な内容だけを文書化する。`prototypeで検証` は後回しにする代わりに、Prototype-driven Mode で prototype を作りながら埋めてもよい。
...
### Prototype-driven Mode

画面、状態、操作、レイアウトのように、言葉で答えると解像度が低くなる論点を、prototype を作りながら埋める場合に使う。ユーザーが「見ないと決められない」と判断した場合もこのモードにする。プロダクトの目的・範囲、品質条件、開発の流れのように画面に出ない論点や、何を作るかがまだ決められない段階では使わず、先に New Mode で前提を固める。

1. 対象（ユーザーフロー、画面、補助面）と、今回確かめる項目を決める。確かめる項目は `project-documents/documentation-policy.md` の「prototypeの運用」から選び、prototype README の「確認観点」に書く。
2. `prototype-workflow` の手順で prototype を作る・直す。
3. 作る途中で仕様に無い判断を下したら、その場で記録する（状態の見せ方、戻る操作の挙動、確定のタイミングなど）。
4. 区切りごとに、記録した判断を文書の更新案として提示する。反映先は方針ファイルの「内容を書く場所の決め方」で選び、ステータスは `DS02` にする。
5. ユーザーの合意を得てから文書に反映し、prototype README の「確認観点」に結果（見つけた問題と、戻した文書）を残す。
6. 確かめた範囲の判断が固まったら、`DS02` から `DS03` へ上げるかを確認する。

- prototype の挙動を、自動的に正本にしない。
- 1回の区切りで扱う範囲を小さくし、prototype の変更と文書の更新案を行き来しやすくする。
...
- 何をなぜ作るか
- 対象ユーザー、対象外ユーザー
- 人物像（特徴と利用文脈）と、人物像ごとの仮説（求めていること、そこから導く UI・情報設計・非機能の方向性、根拠が観察・データ・推測のどれか）。仮説は1人に複数あるものとして聞き、仮説ごとにエントリを分ける。方向性を書かずに仮説だけで終わらせない（書き方は `project-documents/documentation-policy.md` の「product」に従う）
- 利用文脈、利用頻度、代替手段
- 提供価値、価値が崩れる条件
- MVP、対象外、後回し、制約
- 対応環境、レスポンシブの最低範囲
...
非機能要件は2段で聞く。1段目では、`project-documents/documentation-policy.md` の「品質特性の選び方」の表から、プロダクトの目的と提供対象に照らして検討する品質特性を選び、品質特性ごとに採用・後で決める・対象外を決めて、非機能要求の「品質特性の水準」のエントリ（`QLT`）にする。採用するものは、表の選べる水準から基準と水準を決める。2段目では、表に無い条件や表の水準より細かい条件を「品質条件・制約」のエントリ（`NFR`）にし、関係する品質特性があれば `QLT` の ID で結ぶ。表に無い品質特性は ISO/IEC 25010 の観点で探す。水準は後から気づくと戻しにくい最低ラインを優先し、根拠なく高い水準を選ばない。
...
5. 反映先文書と変更しない範囲を提示する。反映先は`project-documents/documentation-policy.md` の「内容を書く場所の決め方」に従って既存の雛形の節・項目から選ぶ。新しいエントリを足すとき・エントリを参照するときは、`project-documents/documentation-policy.md` の「エントリの ID」に従い、次の番号は `pdocs id <project> <接頭辞>` で確かめる。
