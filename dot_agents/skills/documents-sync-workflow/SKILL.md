...
- template変更時は方針ファイルの採用条件と既存プロダクトの実態を確認する。プロダクト文書と雛形の一致は CI の `pdocs check` が検査するので、変更のたびに自分で実行せず、必要なら CI の結果を参照する（project-documents の `AGENTS.md` の「検査」）。移行途中は元情報と未完了範囲を記録する。
