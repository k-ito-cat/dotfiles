...
5. 各まとまりを粒度判定チェックで自己レビューする
6. 各まとまりごとにコミットメッセージを作る
7. コミット前に、予定する分割方針を確認する
8. 提示した対象範囲、コミット分割、コミットメッセージでステージングし、コミットする
...
同じ製品、機能領域、ファイル、設定カテゴリに属することと、変更目的が同じことを混同しない。
機能追加、既定値の変更、有効・無効の切り替えなどが独立して成立する場合は、同じ対象領域でも別コミットに分ける。

分割案を提示する前に、各 hunk について次を自己確認する。

- この変更は、同じコミットに含める他の変更の目的を成立させるために必要か
- この変更だけを revert しても、他の変更が意味を保つか
- 予定する subject が、この変更を含む全 hunk の意図を自然に説明できるか
- 対象領域が同じという理由だけでまとめていないか

この判定はエージェント側で行い、コミット粒度の選択をユーザーに委ねない。
ユーザーには自己レビュー後の確定案を提示し、その案でコミットしてよいかだけを確認する。
...
- 意味のある変更では、subject の後に空行を置き、本文を2〜3行で書く
- 本文には、何を具体的に変更したかを書く
- 設定追加では、追加した設定項目・値・対象を具体的に書く
- コード変更では、追加・変更したものに加え、差分または依頼から理由が分かる場合だけ、その理由を書く。推測で補わない
- 軽微なミス修正、README、ドキュメント、記事、文章だけを変更するコミットには本文を付けない
- subject と重複する抽象表現、実装手順の羅列、長文は避ける
...
- コミット粒度をどうするかユーザーに質問しない。差分を分析して目的単位の分割を確定してから提示する
