...
description: 日本語の技術記事・技術文書を添削するときに使う。「添削」「ブログ添削」と言われたら起動する。整形、段落と論証の構成、論証の厳密さ、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除、見出しの付け方を規範として点検する。対象は記事の本文のみで、`## メモ` 欄は対象外。指摘は本文を書き換えず `> [!REVIEW]` で対象ブロックの直下に差し込む。
---
...
規範の本文は原文のまま。この節だけが astro-markdown-blog 向けの追記である。

- 起動語は「添削」「ブログ添削」。
- 対象は `src/content/posts/blog/*.md` の本文、つまり `## メモ` より上だけ。
- `## メモ` 欄は対象外。メモは原文のまま保存する運用なので、規範を当てて書き換えない。
- 本文を直接書き換えない。指摘は対象ブロックの直下へ `> [!REVIEW]` のコールアウトで差し込む。記事全体への指摘は `> [!OUTLINE]` を本文の冒頭に一つだけ置く。どちらも公開ビルドでは取り除かれる。
- 1つの `> [!REVIEW]` には1つの指摘だけを書く。どの規範に反しているかと、書き換えの候補を添える。
- 添削は一時的な書き込みである。`status: published` の記事では本番ビルド時に自動で取り除かれる（`src/plugins/remarkReviewCallout.mjs`）。
- 公開中の記事（`status: published`）を添削したら、`status` を `draft` に変える。`draft` は「公開中（WIP）」であり、本番ページは生成されるが本文は出ない。添削の途中の原稿を、公開済みのまま置かない。
- 規範のうち「整形」節（一文一行、中黒・ダッシュの禁止）は既存記事と衝突しうる。適用するかどうかを添削の冒頭で確認する。

### 記事全体の構成をレビューする

この規範の本文は書籍の章を対象にしており、構成が決まったあとの推敲しか扱わない。
ブログ記事は一本で完結するため、並び自体の妥当性も見る。
一文単位の指摘（`> [!REVIEW]`）とは別に、本文の冒頭へ `> [!OUTLINE]` を一つだけ置き、次の六つを書く。

- **記事が答える問い**：一文で言えるか。言えないなら構成が決まっていない。
- **入り**：最初の段落で、誰のどの問題を扱う記事かが読者に伝わるか。
- **節の並び**：読者が抱く疑問の順になっているか。
- **結び**：冒頭で立てた問いに答えて終わっているか。事務的な情報で終わっていないか。
- **足りない節の候補**：読者が次に抱く疑問のうち、記事が答えていないもの。本文にすでに素材があるものを優先する。
- **記事の型**：レビュー、作業記録、論考のどれか。型に合わない規範は当てない。

足りない節を提案するときは、なぜ読者がそれを求めるかを一文添える。
根拠を書けないものは提案しない。

### 記法の使いどころをレビューする

このブログには記法が用意されている（正本は `src/components/preview/PreviewBodyEditor.astro` の `SNIPPET_GROUPS`）。
それぞれに用途の注記があるので、地の文がその役割を担っているのに記法で支えていない箇所を探し、置き換えを提案する。

- **アラート**：注意や前提が地の文に埋もれている箇所。用途は注記のとおりに選ぶ（NOTE は流し読みでも目に留めたい補足、TIP はより簡単にやる方法、IMPORTANT は目的の達成に欠かせない前提、WARNING は今すぐ注意が要ること、CAUTION は操作に伴う危険）。
- **テーブル**：同じ観点の項目が三つ以上、文や箇条書きで並んでいる箇所。
- **コード**：地の文に混ざったコマンド、キー、設定値、ファイル名。
- **トグル**：長いログや、読み飛ばしても本筋を追える補助情報。
- **画像のキャプションと alt**：alt が空の画像。alt は読み上げと代替表示、キャプションは画面に出る説明で、役割が違う。
- **リンクカード**：記事の主題そのものを指す URL の単独行。

提案には節度を求める。

- 装飾のための装飾を提案しない。今の文のどこが支えられていないかを必ず書く。
- 地の文で足りるものは置き換えない。「演出の抑制」の規範と衝突しない範囲にとどめる。
- アラートは一記事に二つか三つまで、マーカーと太字は論理の要所だけにする。
- すでに置かれた装飾が過剰なら、外す提案もする。
