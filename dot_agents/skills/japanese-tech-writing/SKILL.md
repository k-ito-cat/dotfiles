...
記法の一覧、書き方、使いどころは `src/components/preview/PreviewBodyEditor.astro` の `SNIPPET_GROUPS` を正本とする。
記法は増える。ここへ一覧を書き写さず、レビューのたびに全項目を読んでから照合する。

見るのは次の3点。

1. **書式**：使っている記法が、注記に書かれた書き方どおりか。テンプレと違う並び、欠けた項目、記法ページに無い自己流の書き方がないか。
2. **不足**：記法で支えるべき箇所を地の文が担っていないか。各項目の注記が用途と目安を持っているので、それに照らして探す。
3. **過剰**：すでに置かれた記法が多すぎないか。外す提案もする。

補足情報をどの層で扱うかは `docs/design/components.md` の「補足情報の分担」を正本とする。
語の説明が脚注に、主張の根拠が用語の注釈に入っていないかを見る。同じ出典が複数の層へ重複していないかも見る。
